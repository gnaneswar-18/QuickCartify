...
const Category = () => {
  ...
  return (
    <section className=" overflow-y-auto " style={{ maxHeight: "80vh" }}>
      <div className="z-100 p-4 bg-white shadow flex justify-between items-center sticky top-0 ">
        ...
        <div className=" p-5">
          <div className="flex flex-wrap gap-6 justify-start items-center">
            {categorydata.map((category) => (
              <div
                key={category._id}
                className=" w-56 h-66 rounded-lg shadow-md hover:shadow-lg border border-gray-200 p-4 flex flex-col items-center"
              >

                <img
                  src={category.image}
                  alt={category.name}
                  className="w-4/5 h-38 object-cover rounded-md mb-3 "
                />
